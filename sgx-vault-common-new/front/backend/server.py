...
def vault_create(pin: str) -> dict:
    """Menu [1] Create vault — new App requires PIN setup."""
    child = _ensure_app()
...
def vault_load(pin: str) -> dict:
    """Menu [2] Load vault — new App requires PIN login after unseal."""
    child = _ensure_app()
    global _vault_loaded
    with _child_lock:
        _send_command(child, "2")

        # Wait for PIN prompt, file-not-found, or immediate error
        idx = child.expect([
            "PIN: ",            # 0 – vault loaded, waiting for PIN
            "File '",           # 1 – sealed file not found
            "Error:",           # 2 – other error
            pexpect.TIMEOUT,    # 3
        ])
        if idx == 1:
            child.expect("Choice:")
            return {"ok": False, "error": "Sealed file not found"}
        if idx == 2:
            child.expect("Choice:")
            return {"ok": False, "error": "Load failed (corrupted or wrong platform)"}
        if idx != 0:
            return {"ok": False, "error": "Timeout"}

        # idx == 0: PIN prompt — enter PIN and handle retry loop
        child.sendline(pin)
        while True:
            idx2 = child.expect([
                "Login successful",     # 0 – authenticated
                "PIN: ",                # 1 – wrong PIN, re-prompted
                "Choice:",              # 2 – exhausted attempts, back to menu
                pexpect.TIMEOUT,        # 3
            ])
            if idx2 == 0:
                child.expect("Choice:")
                ...
                # Wrong PIN — app printed "Invalid PIN. X attempts remaining."
                # and is now showing "PIN: " again; retry
                child.sendline(pin)
            elif idx2 == 2:
                return {"ok": False, "error": "Too many failed attempts. Vault locked."}
            else:
                return {"ok": False, "error": "Timeout during PIN entry"}
...
def api_create():
    ...
    if not pin or len(pin) < 4 or len(pin) > 8:
        return jsonify({"ok": False, "error": "PIN must be 4-8 characters"}), 400
    return jsonify(vault_create(pin))
