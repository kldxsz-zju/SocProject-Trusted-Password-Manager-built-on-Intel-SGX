...
{
    std::string pin, pin_confirm;
    printf("Set a PIN for the vault (4-8 characters): ");
    std::getline(std::cin, pin);
    if (pin.empty() || pin.length() < 4 || pin.length() > 8)
    {
        printf("PIN must be 4-8 characters.\n");
        return VAULT_ERR_INVALID_PARAMETER;
    }
    printf("Confirm PIN: ");
    std::getline(std::cin, pin_confirm);
    if (pin != pin_confirm)
    {
    ...
    int ret = 0;
    sgx_status_t st = ecall_vault_create(g_eid, &ret, pin.c_str());
    if (st != SGX_SUCCESS)
    {
...
static int do_load()
{
    ...
    if (ret != VAULT_OK)
        return ret;
    printf("Vault loaded successfully. Please enter your PIN to unlock.\n");
    for (int attempts = 0; attempts < 3; attempts++)
    {
        ...
        std::getline(std::cin, pin);
        int login_ret = 0;
        st = ecall_vault_login(g_eid, &login_ret, pin.c_str());
        if (st != SGX_SUCCESS)
        {
            printf("ECALL failed: 0x%x\n", st);
            return -1;
        }
        if (login_ret == VAULT_OK)
        {
    ...
    printf("Too many failed attempts. Vault remains locked.\n");
...
static int do_list()
{
    ...
    if (ret != VAULT_ERR_BUFFER_TOO_SMALL && ret != VAULT_OK)
        return ret;
...
static void print_error(int code)
{
    ...
    case VAULT_OK:
        printf("OK.\n");
        break;
    case VAULT_ERR_INVALID_PARAMETER:
        printf("Error: Invalid parameter.\n");
        break;
    case VAULT_ERR_NOT_INITIALIZED:
        printf("Error: Vault not initialized. Create or load first.\n");
        break;
    case VAULT_ERR_ALREADY_INITIALIZED:
        printf("Error: Vault already initialized.\n");
        break;
    case VAULT_ERR_DUPLICATE_SERVICE:
        printf("Error: Service name already exists.\n");
        break;
    ...
    case VAULT_ERR_FULL:
        ...
        break;
    case VAULT_ERR_BUFFER_TOO_SMALL:
        printf("Error: Buffer too small.\n");
        break;
    case VAULT_ERR_SEAL:
        printf("Error: Seal operation failed.\n");
        break;
    case VAULT_ERR_UNSEAL:
        printf("Error: Unseal operation failed (wrong platform/enclave or corrupted file).\n");
        break;
    case VAULT_ERR_FORMAT:
        printf("Error: Unexpected sealed data format.\n");
        break;
    case VAULT_ERR_CORRUPT:
        printf("Error: Sealed data is corrupted or tampered.\n");
        break;
    case VAULT_ERR_INTERNAL:
        printf("Error: Internal error.\n");
        break;
    case VAULT_ERR_LOCKED:
        printf("Error: Vault locked.\n");
        break;
    case VAULT_ERR_AUTH_FAILED:
...
static long get_page_faults()
{
    ...
    return usage.ru_minflt + usage.ru_majflt; // 缺页次数
}
