...
namespace
{
    ...
    static bool validate_wire(const VaultWire &candidate)
    {
        ...
        for (uint32_t i = 0; i < VAULT_MAX_CREDENTIALS; ++i)
        {
            ...
            if (!terminated(c.service, sizeof(c.service)) || c.service[0] == '\0' ||
                ...
                !terminated(c.password, sizeof(c.password)))
                return false;
            for (uint32_t j = i + 1; j < VAULT_MAX_CREDENTIALS; ++j)
            {
                ...
                if (other.in_use == 1 && terminated(other.service, sizeof(other.service)) &&
                    strcmp(c.service, other.service) == 0)
                    return false;
            }
        }
    ...
    static void hash_pin(const uint8_t *salt, size_t salt_len,
                         const char *pin, size_t pin_len,
                         sgx_sha256_hash_t *hash_out)
    {
        uint8_t combined[PIN_SALT_SIZE + VAULT_PIN_MAX + 1];
        memcpy(combined, salt, salt_len);
        memcpy(combined + salt_len, pin, pin_len);
        sgx_sha256_msg(combined, salt_len + pin_len, hash_out);
    }
...
int ecall_vault_login(const char *pin)
{
    ...
    sgx_sha256_hash_t computed_hash[PIN_HASH_SIZE];
    hash_pin(g_vault.pin_salt, sizeof(g_vault.pin_salt),
             pin, strlen(pin),
             computed_hash);
    if (memcmp(computed_hash, g_vault.pin_hash, PIN_HASH_SIZE) == 0)
    {
...
int ecall_vault_add(const char *service, const char *username, const char *password)
{
    ...
    if (find_service(service) >= 0)
        return VAULT_ERR_DUPLICATE_SERVICE;
    if (g_vault.credential_count == VAULT_MAX_CREDENTIALS)
...
{
    if (username != nullptr && username_capacity != 0)
        secure_zero(username, username_capacity);
    if (password != nullptr && password_capacity != 0)
        secure_zero(password, password_capacity);
    if (!g_initialized)
        return VAULT_ERR_NOT_INITIALIZED;
    if (!g_unlocked)
        return VAULT_ERR_LOCKED;
    ...
    if (username_capacity <= strnlen(c.username, sizeof(c.username)) ||
        password_capacity <= strnlen(c.password, sizeof(c.password)))
...
int ecall_vault_list(char *output, size_t output_capacity, size_t *required_size)
{
    ...
    *required_size = 1;
    ...
    if (!g_unlocked)
        return VAULT_ERR_LOCKED;
    for (uint32_t i = 0; i < VAULT_MAX_CREDENTIALS; ++i)
        if (g_vault.credentials[i].in_use)
            *required_size += strlen(g_vault.credentials[i].service) + 1;
...
int ecall_vault_seal(uint8_t *sealed, uint32_t sealed_capacity, uint32_t *actual_size)
{
    ...
    if (!g_unlocked)
        return VAULT_ERR_LOCKED;
    if (sealed == nullptr || actual_size == nullptr)
        return VAULT_ERR_INVALID_PARAMETER;
